...
larva_7 <- read_csv(
  "~/student_documents/UBC/Research/Malawi/data/sac pressure, pH series (6, 7, 8)\\pH8/larva 7, raw.csv") %>%
  ...
  filter(pH == 8) %>%
  write_csv(
    "~/student_documents/UBC/Research/Malawi/data/sac pressure, pH series (6, 7, 8)/pH8/area results, thresh calc/larva 7.csv") %>%
  print()
...
larva_8a <- read_csv("~/student_documents/UBC/Research/Malawi/data/sac pressure, pH series (6, 7, 8)\\pH8/larva 8a, raw.csv") %>%
  rename(" " = ...1) %>% #so that the output can be fed into the larger data frame
  arrange(sac)
print(larva_8a)
...
larva_4 <- rbind(larva_8a, larva_8b) %>%
  group_by(sac) %>%
  mutate(pH6_area = Area[1]) %>%
  mutate(pH7_area = Area[2]) %>%
  filter(pH == 8) %>%
